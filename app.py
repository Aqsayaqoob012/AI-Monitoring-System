...
def stable_multiple_persons(num_persons, last_person_count, frame, current):
    MULTI_THRESHOLD = 1.5
...
def gen_frames():
    ...
    last_person_count = 1 if 'last_person_count' not in globals() else last_person_count
    ...
    while running:
        ...
        # ── Multiple persons (stable)
        last_person_count = stable_multiple_persons(num_persons, last_person_count, frame, current)
        ...
        if direction in direction_events:
            pts = fire_event(direction_events[direction])
            if pts:
...
def video_feed():
    global running
    if not running:
        running = True   # ensure stream is running
...
def upload_video():
    ...
    # Start analyzing uploaded video
    cap = cv2.VideoCapture(video_path)
    running = True
    start_time = time.time()
    return "Video uploaded & analysis started"
...
def gen_frames():
    ...
    while running:
        ret, frame = cap.read()
        if not ret:
            break  # video ended or camera error

        frame = cv2.flip(frame, 1)  # optional, only for live camera
        current = round(time.time() - start_time, 2)
        total_frames += 1
        ...
        def stable_multiple_persons(num_persons, last_person_count, frame, current):
            ...
            """
            num_persons      : current YOLO detected persons
            last_person_count: previous frame's person count
            frame            : current frame (for saving screenshots if needed)
            current          : current timestamp (seconds)

            Returns updated last_person_count
            """
            MULTI_THRESHOLD = 1.5  # minimum difference to consider multiple persons
